...
class Tree(UIElement):
    ...
    def __init__(self, tree_camera: TreeCamera, course_tree: CourseTree):
        super().__init__()
    ...
    def draw_tree_visualization(self, tree: CourseTree, x_pos: int, y_pos: int, spacing_factor: int, zoom_factor: int,
                                node_course_code_map: list[tuple[pygame.Rect, str]], ui_screen: pygame.Surface):
        if tree.is_empty():
            return
        else:
            self.draw_node((tree.get_root(), tree.get_grade_requirement()), x_pos, y_pos, zoom_factor, node_course_code_map, ui_screen)
            total_spacing = self.tree_width(tree) * spacing_factor * zoom_factor
            start_x_pos = x_pos - total_spacing // 2  # center children under parent

            for subtree in tree.get_subtrees():
                # horizontally place child node
                subtree_width = self.tree_width(subtree) * spacing_factor * zoom_factor
                ...
                self.draw_tree_visualization(subtree, child_x, y_pos + int(150 * zoom_factor), spacing_factor, zoom_factor,
                                        node_course_code_map, ui_screen)

                start_x_pos += subtree_width

    def draw_node(self, display_vals: tuple[str | None, str], x_pos: int, y_pos: int, screen_zoom_factor: int,
                  node_course_code_map: list[tuple[pygame.Rect, str]], ui_screen: pygame.Surface):
        """
        Draw a node on point (x_pos, y_pos) with text, display_val
        """
        ...
        ui_screen.blit(text_img, text_rect)
...
class MainScreenUI(UIManager):
    ...
    TEXT_OUTPUT = 1
    ERROR_OUTPUT = 2
    BG_PATH = "course_compass_main_v5.png"
    # Instance attributes:
    ...
    def __init__(self):
        ...
        # append to ui_elements list for handle_event to work properly
        # TODO: remove once handle_event is implemented based on panel_output mode
        self.ui_elements.append(self.visualizer_search_field)
        ...
        # append to ui_elements list for handle_event to work properly
        # TODO: remove once handle_event is implemented based on panel_output mode
        self.ui_elements.append(self.summer_offering_button)
        ...
        # append to ui_elements list for handle_event to work properly
        # TODO: remove once handle_event is implemented based on panel_output mode
        self.ui_elements.append(self._tree_ui_element)

    def handle_event(self, ui_event):
        """Handles a UI event"""
        # the default implementation is to call handle_interaction on all elements in the ui_elements list
...
if __name__ == '__main__':
    ...
    while not done:
        ...
        if screen_mode == "start_screen":
            ...
            #     draw_tree_visualization(main_screen_ui.course_tree, tree_camera.x_pos_tree,
            #                             tree_camera.y_pos_tree, 300, tree_camera.zoom_factor,
            #                             tree_camera.node_course_code_map)
            # elif main_screen_ui.panel_output_mode == MainScreenUI.TEXT_OUTPUT:
            #     ...
            # screen.blit(main_screen_ui.get_background_surface(), (0, 0))
